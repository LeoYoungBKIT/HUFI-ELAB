...
export function Dashboard() {
  const snackbarState = useAppSelector(
    (state: RootState) => state.app.snackbarState
  );
  ...
  const getChemicalWarehouseData = async () => {
    const listOfChemicalWarehouse: IChemicalWarehouseType[] =
      await getChemicalWarehouseById(role);
  ...
  const getRegisterGeneralsData = async () => {
    const listOfRegisterGeneral: IRegisterGeneralType[] =
      await getRegisterGenerals("2");
  ...
  const getPurchaseOrderData = async () => {
    const listOfPurchaseOrders: IPurchaseOrderType[] =
  ...
  const getTrainInstructorData = async () => {
    const listOfTrainInstructors: ITrainInstructor[] =
      await getTrainInstructors();
  ...
  const getSuggestNewDevicesData = async () => {
    const listOfSuggestNewDevices: ISuggestNewDeviceType[] =
      await getSuggestNewDevices("2");
  ...
  return (
    <>
      <div className="home">
        ...
        {sidebarItems[9].isOpen &&
          (role === 1 ? (
            <FacultyViewScheduleTable />
          ) : (
            <TeacherViewScheduleTable />
          ))}
