...
const LaboratoryTable: FC = () => {
	...
	const getCommonEditTextFieldProps = useCallback(
		(cell: MRT_Cell<ILaboratoryType>): MRT_ColumnDef<ILaboratoryType>['muiTableBodyCellEditTextFieldProps'] => {
			return {
	...
	const handleSubmitEditModal = async () => {
		...
		if (isUpdatedSuccess) {
			...
			let newListOfLabs = [
				...laboratoriesData.slice(0, updatedIdx),
				updatedRow,
				...laboratoriesData.slice(updatedIdx + 1),
			];
							...
							{columns.map(column =>
								column.id === 'Note' ? (
									<TextareaAutosize
										...
										onChange={e => setUpdatedRow({ ...updatedRow, Note: e.target.value })}
									/>
								) : (
									<TextField
...
type CreateModalProps = {
	...
	columns: MRT_ColumnDef<ILaboratoryType>[];
	handleSubmit: Function;
	onCloseCreateModal: () => void;
};

const CreateModal = ({ isCreateModal, columns, handleSubmit, onCloseCreateModal }: CreateModalProps) => {
	const [createdRow, setCreatedRow] = useState<any>(dummyLaboratoryData);

	const handleSubmitCreateModal = () => {
		handleSubmit(createdRow);
		onCloseCreateModal();
	};
							...
							{columns.map(column =>
								column.id === 'Note' ? (
									<TextareaAutosize
										...
										onChange={debounce(
											e => setCreatedRow({ ...createdRow, Note: e.target.value }),
											300,
										)}
									/>
								) : (
