...
const ExportToLiquidateTable: FC = () => {
    const owner = useAppSelector(state => state.userManager.owner)
    const exportManagementForms = useAppSelector(state => state.exportManagement.listOfExportToLiquidateManagementForms)

    const [isOpenCreateModal, setIsOpenCreateModal] = useState<boolean>(false)
    const [selectedExportToLiquidateManagementForm, setSelectedExportToLiquidateManagementForm] = useState<IExportToLiquidateManagementFormType>()

    const handleOpenCreate = () => {
        setIsOpenCreateModal(true)
    }

    const columns = useRef<ExportToLiquidateManagementFormTypeColumnType[]>([
        { id: 'LiquidateId', header: 'Mã phiếu' },
        ...
        return new DataSource({
            store: new ArrayStore({
                data: exportManagementForms.length > 0 ?
                    exportManagementForms.map((x: any) => ({ ...x, Id: uniqueId('ExportToLiquidateManagementForm_') })) : [],
                key: 'Id',
            }),
        })
                ...
                <>
                        ...
                        >
                            ...
                            <Column type="buttons">
                                <DevButtonGrid
                                    icon="chevrondown"
                                    onClick={(e: any) => {
                                        setSelectedExportToLiquidateManagementForm(e.row.data)
                                    }}
                                />
                            </Column>
                            <Toolbar>
                    ...
                    {selectedExportToLiquidateManagementForm && (
                        <RowExportToLiquidateManagementFormType
                            exportManagementForm={selectedExportToLiquidateManagementForm}
...
export default ExportToLiquidateTable
