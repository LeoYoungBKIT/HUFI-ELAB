...
const commonFieldsShow = [
    ...
    {
        id: "LinkFileRepair",
        header: "Biên bản kiểm tra thiết bị từ Đơn vị phụ trách tiếp nhận sửa chữa",
    },
...
const DialogCreate = ({ isOpen, onClose }: DialogProps) => {
    ...
    const deviceList = useAppSelector(
        (selector) => selector.device.listOfDevices
    );
    const departmentList = useAppSelector(
        (selector) => selector.department.listOfDepartments
    );
    const owner = useAppSelector((state) => state.userManager.owner);

    const [currentCreatedForm, setCurrentCreatedForm] = useState<any>(
        dummyExportToLiquidateManagementForm
    );
    ...
    const getDeviceListOfCreateDepartment = async () => {
        ...
        if (deviceInfoList?.length > 0) {
            let normalizatedDeviceInfoList = deviceInfoList.map((item: any) => {
                let correspondingDeviceType = deviceList.find(
                    (deviceType: any) => deviceType.DeviceId === item.DeviceId
                );

                if (
                    correspondingDeviceType?.DeviceId &&
                    correspondingDeviceType.hasOwnProperty("listDeviceInfo")
                ) {
                    return item.listDeviceInfoId.map((x: any) => {
                        let correspondingDevice =
                            correspondingDeviceType?.listDeviceInfo
                                ? correspondingDeviceType.listDeviceInfo.find(
                                      (y: any) => y.DeviceInfoId === x
                                  )
                                : null;
    ...
    const handleSave = async () => {
        if (!currentCreatedForm?.Title) {
            ...
            currentCreatedForm.listDeviceInfo.forEach(
                (item: any, idx: number) => {
                    if (!item?.DeviceId) {
                        dispatch(
                            setSnackbar({
                                message: `Vui lòng chọn thiết bị ở dòng thứ ${
                                    idx + 1
                                }!`,
                                color: colorsNotifi["error"].color,
                                backgroundColor:
                                    colorsNotifi["error"].background,
                            })
                        );
                        return;
                    }
            ...
            currentCreatedForm?.DepartmentManageId.forEach(
                (item: any, idx: number) => {
                    if (!item) {
                        dispatch(
                            setSnackbar({
                                message: `Vui lòng chọn đơn vị quản lý thiết bị ở dòng thứ ${
                                    idx + 1
                                }!`,
                                color: colorsNotifi["error"].color,
                                backgroundColor:
        ...
        if (
            !currentCreatedForm?.DepartmentManageId.every(
                (item: any) =>
                    item === currentCreatedForm?.DepartmentManageId[0]
            )
        ) {
            dispatch(
                setSnackbar({
                    message:
                        "Vui lòng chọn cùng đơn vị quản lý ở các thiết bị!",
                    color: colorsNotifi["error"].color,
                    backgroundColor: colorsNotifi["error"].background,
                })
            );
            ...
            try {
                ...
                const listOfExportToLiquidateManagementFormTypes: IExportToLiquidateManagementFormType[] =
                    await getListOfLiquidateDeviceForms();
                if (listOfExportToLiquidateManagementFormTypes) {
                    dispatch(
                        setListOfExportToLiquidateManagementForms(
                            listOfExportToLiquidateManagementFormTypes
                        )
                    );
                }
                handleClose();
...
const DataGridFunc = React.memo(function DataGridFunc({
    ...
    const onContentReady = (e: any) => {
        ...
        let newListDeviceInfo = allRows.map((rowItem: any) =>
            Object.assign({}, { DeviceId: rowItem.values[0] })
        );
        setCurrentCreatedForm((prevState: any) =>
            Object.assign(
                ...
                {
                    ...prevState,
                    DepartmentManageId: allRows.map(
                        (row: any) => row.values[5]
                    ),
    ...
    return (
        <DataGrid
            ...
            elementAttr={{
                style: "height: 100%; padding-bottom: 20px; width: 100%; min-width: 600px",
            }}
                ...
                if (col.id === "DeviceName") {
                    ...
                    return (
                        <Column
                            ...
                            caption={col.header}
                            allowEditing={false}
                            cellRender={(data) => <span>{data.text}</span>}
                        />
                    );
                }
            })}
