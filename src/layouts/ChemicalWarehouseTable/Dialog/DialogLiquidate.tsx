...
const DialogLiquidate = ({ isOpen, onClose }: DialogProps) => {
  ...
  const id = 2;
  const [initDataDialog, setInitDataDialog] =
    useState<ILiquidateChemical | null>(null);
        ...
        >
          ...
          >
            <DataGrid
              ...
              elementAttr={{
                style:
                  "height: 100%; padding-bottom: 20px; width: 100%; min-width: 800px",
              }}
            >
...
const DetailTemplate = (props: any) => {
  ...
  function isLiquidateChemical(obj: any): obj is ILiquidateChemical {
    if (obj !== null)
      return (
        "ChemDeptId" in obj && "ChemicalName" in obj && "ChemicalId" in obj
      );
    return false;
  }
...
const DialogCreateLiquidate = ({
  ...
  data?: ILiquidateChemical | [];
  getLiquidateDept: () => Promise<void>;
  type: string;
}) => {
  ...
  const columns = useRef<
    (ColumnsType & {
      colSize: ColumnSizeType;
      required?: boolean;
      readonly?: boolean;
    })[]
  >([
    {
      id: "ExpLiquidateDeptId",
  ...
  useEffect(() => {
    (async () => {
      try {
        let listChemical: ILiquidateChemicalItem[] =
          await getLiquidateDeptChemical(id);
  ...
  useEffect(() => {
    setListChemical((prev) => {
      return prev.filter(
        (x) =>
          selectedChemicals.findIndex((s) => s.ChemDeptId === x.ChemDeptId) ===
          -1
      );
    });
  }, [selectedChemicals]);
  ...
  const handleConfirm = async () => {
    ...
    columns.current.forEach((col) => {
      if (
        `${postData[col.id as keyof typeof postData]}`.trim() === "" &&
        col.required
      ) {
              ...
              <Grid item {...col.colSize} key={col.id}>
                ...
                >
                  ...
                  <Input
                    ...
                    defaultValue={
                      new Date(
                        Number(liquidate[col.id as keyof typeof liquidate]) *
                          1000
                      ).toLocaleDateString("en-CA") !== "1970-01-01"
                        ? new Date(
                            Number(
                              liquidate[col.id as keyof typeof liquidate]
                            ) * 1000
                    ...
                    onChange={(e) =>
                      handleDebouncedInput(
                        col.id,
                        `${Number(new Date("2023-10-09")) / 1000}`
                      )
                    }
                  />
                  {indexError !== -1 && (
                    <FormHelperText>{errors[indexError].msg}</FormHelperText>
                  )}
                </FormControl>
              </Grid>
            );
          ...
          <Box>
            <Autocomplete
              ...
              isOptionEqualToValue={(option, value) =>
                option.ChemDeptId === value.ChemDeptId
              }
